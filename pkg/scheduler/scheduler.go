...
func (s *Scheduler) scheduleOnce(ctx context.Context) {
	...
	if closed {
		// End the run if the work queue has been closed.
	...
	for _, binding := range bindingList.Items {
		...
		if binding.DeletionTimestamp == nil {
			if err := s.client.Delete(ctx, &binding); err != nil {
				klog.ErrorS(err, "failed to delete binding", "ClusterResourceBinding", binding)
				return controller.NewAPIServerError(err)
			}
		}

		// Check if the binding has the scheduler cleanup finalizer.
		if controllerutil.ContainsFinalizer(&binding, fleetv1beta1.SchedulerCRPCleanupFinalizer) {
			// Remove the scheduler cleanup finalizer from the binding.
			controllerutil.RemoveFinalizer(&binding, fleetv1beta1.SchedulerCRPCleanupFinalizer)
			// Update the binding.
			if err := s.client.Update(ctx, &binding); err != nil {
				klog.ErrorS(err, "failed to remove scheduler cleanup finalizer from binding", "ClusterResourceBinding", crpRef)
				return controller.NewAPIServerError(err)
			}
		}
	}
...
// a CRP.
func (s *Scheduler) lookupLatestPolicySnapshot(ctx context.Context, crp *fleetv1beta1.ClusterResourcePlacement) (*fleetv1beta1.ClusterPolicySnapshot, error) {
	crpRef := klog.KObj(crp)

	// Find out the latest policy snapshot associated with the CRP.
	policySnapshotList := &fleetv1beta1.ClusterPolicySnapshotList{}
	...
	if err := s.client.List(ctx, policySnapshotList, listOptions); err != nil {
		...
		return nil, controller.NewAPIServerError(err)
	}
	...
	case len(policySnapshotList.Items) == 0:
		...
		klog.ErrorS(err, "ClusterResourcePlacement", crpRef)
		return nil, err
	case len(policySnapshotList.Items) > 1:
		...
		klog.ErrorS(err, "multiple latest policy snapshots associated with cluster resource placement", "ClusterResourcePlacement", crpRef)
		return nil, err
	default:
